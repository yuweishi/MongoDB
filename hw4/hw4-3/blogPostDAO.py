...
class BlogPostDAO:
    ...
    def __init__(self, database):
        ...
        self.posts.create_index([("tags",1),("date", -1)])
        self.posts.create_index([("permalink",1)])
        self.posts.create_index([("date", -1)])
    ...
    def get_posts_by_tag(self, tag, num_posts):
        ...
        cursor = self.posts.find({'tags':tag}).sort('date', direction=-1).hint([('tags', 1), ('date', -1)]).limit(num_posts)
        l = []
